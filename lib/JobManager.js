...
class JobManager {
  ...
  startJobs() {
    this._jobs.forEach((job) => {
      try {
        job.start();
        logger.info({ event: 'review-app-manager', job: job.name, message: `Started job ${job.name} with cron time "${job.cron.cronTime}"` });
      } catch (err) {
        logger.error({ event: 'review-app-manager', job: job.name, message: err });
      }
    });
  }
  ...
  stopJobs() {
    this._jobs.forEach((job) => {
      job.stop();
      logger.info({ event: 'review-app-manager', job: job.name, message: `Stopped job ${job.name}` });
    });
  }
