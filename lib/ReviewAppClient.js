...
class ReviewAppClient {
  ...
  async _pollOperationStatus(operation) {
    ...
    const executePoll = async (resolve) => {
      ...
      if (operation.status === 'error') {
        logger.error({ event: 'review-app', message: operation.error });
        return resolve();
      }
      if (this._pollMaxAttempts && attempts >= this._pollMaxAttempts) {
        logger.error({ event: 'review-app', message: 'Exceeded max attempts' });
        return resolve();
      }
  ...
  async scale(app, formation) {
    const scalingoClient = await this._getClient();
    logger.info({ event: 'review-app', message: `Scaling app ${app.name} to ${formation[0].amount} container(s)…` });

    try {
      const { operation } = await scalingoClient.Containers.scale(app.name, formation);
      // TODO get value and check if error to not write successfully message
      await this._pollOperationStatus(operation);
      logger.info({ event: 'review-app', message: `App ${app.name} scaled successfully` });
    } catch (err) {
      if ((err._data && err._data.error === 'no change in containers formation') || err === 'no change in containers formation') {
        logger.warn({ event: 'review-app', message: `App ${app.name} not scaled due to unchanged formation.` });
      } else if (err.status === 422) {
        logger.warn({ event: 'review-app', job: app.name, message: err });
      } else {
        logger.error({ event: 'review-app', job: app.name, message: err });
        throw err;
      }
    }
  }
